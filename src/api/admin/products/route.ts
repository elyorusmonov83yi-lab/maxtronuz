  ...
  try {
    ...
    const products = rows.map((p: any) => {
      ...
      return {
        ...
        brandId: p.brand_id || 'b-maxtron', // 🌟 Brand ID
        model: p.model || '',
...
// 2. POST: Mahsulotni saqlash yoki yangilash
router.post('/', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const brandId = String(prod.brandId || 'b-maxtron').trim(); // 🌟 Yangi brandId qabul qilish
