      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              {categories.slice(0, 6).map((cat) => (
                <li key={cat.id}>
                  <Link 
                    href={getLocalizedUrl(`/catalog?category=${cat.id}`)} 
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              <li>
                <Link href={getLocalizedUrl('/')} className="hover:text-blue-400 transition">
                  {t.nav_home || 'Bosh sahifa'}
                </Link>
              </li>
              <li>
                <Link href={getLocalizedUrl('/catalog')} className="hover:text-blue-400 transition">
                  {t.nav_catalog || 'Katalog'}
                </Link>
              </li>
              <li>
                <Link href={getLocalizedUrl('/equipment-finder')} className="hover:text-blue-400 transition flex items-center gap-1.5 text-cyan-400">
                  <span>{currentLang === 'ru' ? 'Подбор КИПиА' : 'Uskuna tanlash'}</span>
                </Link>
              </li>
              <li>
                <Link href={getLocalizedUrl('/certificates')} className="hover:text-blue-400 transition">
                  {t.nav_certificates || 'Sertifikatlar'}
                </Link>
              </li>
              <li>
                <Link href={getLocalizedUrl('/about')} className="hover:text-blue-400 transition">
                  {t.nav_about || 'Biz haqimizda'}
                </Link>
              </li>
              <li>
                <Link href={getLocalizedUrl('/contact')} className="hover:text-blue-400 transition">
                  {t.nav_contact || 'Aloqa'}
                </Link>
              </li>
              ...
              {footerCustomPages.map((page) => {
                ...
                return (
                  <li key={page.id}>
                    <Link href={pageUrl} className="hover:text-blue-400 transition flex items-center gap-1.5 text-gray-300">
                      <span>{pageTitle}</span>
                    </Link>
                  </li>
                );
        ...
        <div className="pt-10 mt-10 border-t border-gray-800/80 flex flex-col sm:flex-row items-center justify-between text-xs text-gray-500 gap-4">
          <div>
            © 2026 MAXTRON Group LLC. {t.footer_rights || 'Barcha huquqlar himoyalangan.'}
          </div>
          <div className="flex items-center space-x-6">
            <Link href={getLocalizedUrl('/certificates')} className="hover:text-gray-400 transition">
              {t.footer_privacy || (currentLang === 'ru' ? 'Сертификация' : 'Sertifikatlar')}
            </Link>
            <span>•</span>
            <Link href={getLocalizedUrl('/contact')} className="hover:text-gray-400 transition">
              {t.footer_terms || (currentLang === 'ru' ? 'Гарантия и сервис' : 'Kafolat va servis')}
            </Link>
            <span>•</span>
            <Link href="/admin" className="text-gray-500 hover:text-blue-400 transition font-medium">
              Admin Portal
            </Link>
          </div>
        </div>
