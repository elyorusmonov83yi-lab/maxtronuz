...
// Meta teglarni xavfsiz yangilash yoki yaratish funksiyasi
...
export const CertificatesView: React.FC<CertificatesViewProps> = ({
  ...
  // 🌟 To'liq Dinamik SEO, Meta va OpenGraph teglarni o'rnatish
  const applySeoTags = () => {
    ...
    const titleText = getLocalizedText(
      certSeo?.title || {
        uz: "Sertifikatlar va Ruxsatnomalar — MAXTRON",
        ru: "Сертификаты и Разрешительная документация — MAXTRON"
      },
      currentLang
    );
    ...
    const descText = getLocalizedText(
      certSeo?.description || {
        uz: "O'zstandart Davlat reestri guvohliklari, muvofiqlik va ISO 9001 sifat sertifikatlari.",
        ru: "Свидетельства Госреестра Узстандарт, сертификаты соответствия и ISO 9001."
      },
      currentLang
    );
    ...
    // 3. Meta Keywords
    const keywordsText = getLocalizedText(
      certSeo?.keywords || {
        uz: "sertifikatlar, maxtron, uzstandard, ISO 9001, rasmiy diler, pasport, guvohnoma",
        ru: "сертификаты, maxtron, узстандарт, ISO 9001, дилерский сертификат, поверка приборов"
      },
      currentLang
    );
    setMetaTag('name', 'keywords', keywordsText);

    // 4. OpenGraph & Twitter Card
    const ogTitle = getLocalizedText(certSeo?.ogTitle || certSeo?.title, currentLang) || titleText;
    const ogDesc = getLocalizedText(certSeo?.ogDescription || certSeo?.description, currentLang) || descText;
    
    // OG Image uchun to'liq URL shakllantirish (Telegram/Facebook talabi)
    let ogImageUrl = certSeo?.ogImage || seoSettings?.ogImageUrl || '/logo.png';
  ...
  useEffect(() => {
    ...
    // SEO sozlamalari yangilanganida teglarni ham darhol yangilash
    const handleSeoUpdate = () => applySeoTags();
    window.addEventListener('maxtron_seo_updated', handleSeoUpdate);
    return () => window.removeEventListener('maxtron_seo_updated', handleSeoUpdate);
  }, [currentLang]);
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button
                    onClick={() => onSelectCertificate(cert)}
                    className="flex-1 inline-flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-gray-800 hover:bg-blue-600 text-xs font-semibold text-white transition shadow-sm"
                  >
                  ...
                  {hasPdf ? (
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-1.5 py-2.5 px-3.5 rounded-xl bg-rose-600/20 text-rose-300 hover:bg-rose-600 hover:text-white border border-rose-500/40 text-xs font-semibold transition"
