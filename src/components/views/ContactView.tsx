...
export const ContactView: React.FC<ContactViewProps> = ({ currentLang, onShowToast }) => {
  ...
  // SEO & OpenGraph dinamik yuklash
  useEffect(() => {
    const seoSettings = StorageService.getSeoSettings();
    const contactSeo = seoSettings?.pageSeo?.contact;

    const titleText = contactSeo?.title?.[currentLang] || (currentLang === 'ru' 
      ? 'Контакты — MAXTRON Industrial Supply' 
      : 'Aloqa — MAXTRON Industrial Supply');
    document.title = titleText;

    const descText = contactSeo?.description?.[currentLang] || (currentLang === 'ru'
      ? 'Свяжитесь с MAXTRON: телефон отдела продаж, адрес главного офиса в Ташкенте и банковские реквизиты.'
      : "MAXTRON bilan bog'laning: savdo bo'limi telefoni, Toshkentdagi bosh ofis manzili va bank rekvizitlari.");
    setMetaTag('name', 'description', descText);

    if (contactSeo?.keywords?.[currentLang]) {
      setMetaTag('name', 'keywords', contactSeo.keywords[currentLang]);
    }

    const ogTitle = contactSeo?.ogTitle?.[currentLang] || titleText;
    const ogDesc = contactSeo?.ogDescription?.[currentLang] || descText;
    let ogImageUrl = contactSeo?.ogImage || seoSettings?.ogImageUrl || '/logo.png';
    if (ogImageUrl && !ogImageUrl.startsWith('http')) {
      ogImageUrl = window.location.origin + (ogImageUrl.startsWith('/') ? '' : '/') + ogImageUrl;
    }
  ...
  const workingHours = getLocalized(contactSettings.workingHours, currentLang === 'ru' ? 'Понедельник — Суббота: с 09:00 до 18:00 (Воскресенье выходной)' : 'Dushanba — Shanba: 09:00 dan 18:00 gacha (Yakshanba dam olish kuni)');
  ...
  return (
    <div className="py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        {/* O'ng tomondagi Bosh Ofis & Ijtimoiy Tarmoqlar */}
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="p-6 rounded-3xl bg-gray-900 border border-gray-800 space-y-5 shadow-xl">
            ...
            <div className="space-y-4 text-xs">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <div className="text-gray-200 font-semibold mt-0.5 leading-relaxed">
                    {mainAddress}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <div className="text-gray-200 font-semibold mt-0.5 font-mono space-y-0.5">
                    ...
                    {(contactSettings.fastPhone || contactSettings.phoneSecondary) && (
                      <div>
                        <a href={`tel:${(contactSettings.fastPhone || contactSettings.phoneSecondary || '').replace(/[^0-9+]/g, '')}`} className="text-emerald-400 hover:underline">
                          {contactSettings.fastPhone || contactSettings.phoneSecondary}
                        </a>
                        <span className="text-[10px] text-gray-400 ml-1.5">
                          ({currentLang === 'ru' ? 'Оперативный отдел' : 'Tezkor bo‘lim'})
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <div className="text-gray-200 font-medium mt-0.5 leading-relaxed">
                    {workingHours}
                  </div>
                </div>
              </div>
            ...
            {/* Barcha Ijtimoiy Tarmoqlar & Mesenjerlar */}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                ...
                {contactSettings.facebookUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="p-2 rounded-xl bg-blue-700/10 hover:bg-blue-700/20 border border-blue-600/20 text-blue-300 text-xs font-semibold flex items-center justify-center gap-1.5 transition"
                  >
                ...
                {contactSettings.linkedinUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="p-2 rounded-xl bg-sky-600/10 hover:bg-sky-600/20 border border-sky-500/20 text-sky-400 text-xs font-semibold flex items-center justify-center gap-1.5 transition"
                  >
                ...
                {contactSettings.youtubeUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="p-2 rounded-xl bg-red-600/10 hover:bg-red-600/20 border border-red-500/20 text-red-400 text-xs font-semibold flex items-center justify-center gap-1.5 transition"
                  >
          ...
          <div className="p-6 rounded-3xl bg-gray-900/80 border border-gray-800 space-y-3 shadow-lg">
            ...
            <div className="text-xs text-gray-300 space-y-1.5 font-mono">
              ...
              <div>
                <span className="text-gray-500 font-sans">{currentLang === 'ru' ? 'Код плательщика НДС:' : 'QQS to‘lovchisi kodi:'}</span>{' '}
                <strong className="text-emerald-400">{contactSettings.vatNumber || '308991204/12'}</strong>
              </div>
            </div>
          </div>
