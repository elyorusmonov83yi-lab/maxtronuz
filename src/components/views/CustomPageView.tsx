...
export const CustomPageView: React.FC<CustomPageViewProps> = ({
  ...
  const { slug, langPrefix } = useParams<{ slug?: string; langPrefix?: string }>();
  ...
  useEffect(() => {
    if (page) {
      const title = getLocalizedText(page.metaTitle, currentLang) || getLocalizedText(page.title, currentLang);
      const description = getLocalizedText(page.metaDescription, currentLang) || getLocalizedText(page.subtitle, currentLang) || getLocalizedText(page.content, currentLang).slice(0, 160);
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap">
          <Link
            to={getLocalizedUrl('/catalog')}
            className="px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shadow-lg shadow-blue-500/20 transition"
          >
            {t.nav_catalog}
          </Link>
          <Link
            to={getLocalizedUrl('/')}
            className="px-6 py-3 rounded-xl bg-gray-900 hover:bg-gray-800 text-gray-300 border border-gray-800 font-semibold text-xs transition"
          >
            {t.nav_home}
          </Link>
        </div>
      ...
      <div className="bg-gray-950/80 border-b border-gray-800/80 py-3.5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between text-xs text-gray-400">
          <div className="flex items-center space-x-2 overflow-x-auto whitespace-nowrap scrollbar-none">
            <Link to={getLocalizedUrl('/')} className="hover:text-blue-400 transition flex items-center gap-1">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handleShare}
              className="hidden sm:inline-flex items-center gap-1.5 text-gray-400 hover:text-white transition text-xs font-semibold px-2.5 py-1 rounded-lg hover:bg-gray-900 border border-transparent hover:border-gray-800"
              title="Саҳифани улашиш"
            >
        ...
        <div className="p-8 sm:p-10 rounded-3xl bg-gradient-to-r from-blue-950/80 via-gray-900 to-cyan-950/80 border border-blue-500/30 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-3 shrink-0 flex-wrap">
            <button
              ...
              className="px-6 py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs sm:text-sm shadow-xl shadow-blue-500/30 transition transform hover:-translate-y-0.5"
            >
              {t.cta_btn || 'Тижорат таклифи олиш'}
            </button>
            <a
          ...
          <div className="border-t border-gray-800 pt-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {otherPages.map((op) => (
                <Link
                  key={op.id}
                  to={getLocalizedUrl(`/page/${op.slug}`)}
                  className="p-4 rounded-2xl bg-gray-900/60 hover:bg-gray-900 border border-gray-800 hover:border-blue-500/30 transition flex items-center justify-between group"
                >
