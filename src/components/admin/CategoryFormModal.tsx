...
import { CategoryInfo, LocalizedString } from '../../types';
...
interface CategoryFormModalProps {
  ...
  categories?: CategoryInfo[]; // Ota toifalarni tanlash uchun
  onSave: (category: CategoryInfo) => void | Promise<void>;
}

// Matnni avtomatik toza URL Slug ga aylantiruvchi funksiya
...
// Universal Ikonka chiqaruvchi (Lucide yoki maxsus SVG)
...
export const CategoryFormModal: React.FC<CategoryFormModalProps> = ({
    ...
    if (isOpen) {
      if (categoryToEdit) {
        ...
        setFormData({
          ...
          slug: {
            uz: categoryToEdit.slug?.uz || categoryToEdit.id || '',
            ru: categoryToEdit.slug?.ru || categoryToEdit.id || ''
          },
  ...
  // Ota toifa bo'la oladigan toifalar (O'zi va o'zining ichki bolalari bo'lmagan asosiy toifalar)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-gray-950/85 backdrop-blur-md overflow-y-auto animate-in fade-in">
      <div className="w-full max-w-5xl bg-gray-900 border border-gray-800 rounded-3xl shadow-2xl overflow-hidden my-auto flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white leading-tight">
                {categoryToEdit ? (activeLang === 'ru' ? 'Редактировать категорию' : 'Kategoriyani tahrirlash') : (activeLang === 'ru' ? 'Добавить новую категорию' : 'Yangi kategoriya qo‘shish')}
              </h2>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-6 flex-1">
          ...
          {activeTab === 'general' && (
            <div className="space-y-5 animate-in fade-in duration-150">
              ...
              {/* 🌟 1. OTA TOIFA TANLASH (PARENT CATEGORY SELECTOR) */}
                  ...
                  {availableParentCategories.map((pCat) => {
                    ...
                    return (
                      <option key={pCat.id} value={pCat.id}>
                        ↳ 📄 {activeLang === 'ru' ? (nameRu || nameUz) : (nameUz || nameRu)} ({activeLang === 'ru' ? 'Подкатегория' : 'Kichik toifa'})
                      </option>
                    );
                  })}
                </select>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-gray-800/80">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="px-3 py-1.5 rounded-xl bg-gray-900 border border-gray-800 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 font-mono w-48"
                    />
                ...
                <div className="space-y-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-1.5 rounded-xl bg-gray-900 border border-gray-800 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-blue-500"
                    />
                  </div>
