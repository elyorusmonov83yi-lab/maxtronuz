...
export const AdminProducts: React.FC<AdminProductsProps> = ({
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      const slugRu = (typeof p.slug === 'object' ? p.slug?.ru : p.slug || '').toLowerCase();
      const slugUz = (typeof p.slug === 'object' ? p.slug?.uz : p.id || '').toLowerCase();
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
        <div className="flex flex-1 flex-col sm:flex-row items-center gap-3">
          ...
          <div className="relative w-full sm:max-w-xs">
            ...
            <input
              type="text"
              placeholder="Qidirish (RU/UZ, model, brend)..."
              value={searchQuery}
          ...
          <div className="relative w-full sm:w-44">
            <Tag className="w-3.5 h-3.5 text-gray-500 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
            <select
              value={selectedBrand}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-gray-300">
              <thead className="bg-gray-950/80 text-gray-400 border-b border-gray-800 text-[11px] uppercase tracking-wider font-semibold">
                <tr>
                  <th className="py-4 px-4 w-16 text-center">Rasm</th>
                  <th className="py-4 px-4">Mahsulot nomi (RU / UZ)</th>
                  <th className="py-4 px-4">Brend</th>
                  <th className="py-4 px-4">Model / Kod</th>
                  <th className="py-4 px-4">Kategoriya</th>
                  <th className="py-4 px-4">Tavsif (RU/UZ)</th>
              ...
              <tbody className="divide-y divide-gray-800/60 font-medium">
                {filteredProducts.map((p) => {
                  ...
                  const hasDescRu = Boolean(descRu && String(descRu).trim());
                  const hasDescUz = Boolean(descUz && String(descUz).trim());

                  const slugRu = typeof p.slug === 'object' ? p.slug?.ru || p.id : p.slug || p.id;
                  const slugUz = typeof p.slug === 'object' ? p.slug?.uz : undefined;
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-1.5">
                          <span 
                            ...
                            title={hasDescRu ? 'RU tavsif kiritilgan' : 'RU tavsif kiritilmagan'}
                          >
                            RU
                          </span>
                          <span 
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          {p.inStock ? (
                            ...
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-400 text-[10px] border border-amber-500/20">
                              <span>Buyurtmaga</span>
                            </span>
                          )}

                          {p.isPopular && (
                            <span className="p-1 rounded-md bg-purple-500/10 text-purple-400 border border-purple-500/20" title="Ommabop uskuna">
