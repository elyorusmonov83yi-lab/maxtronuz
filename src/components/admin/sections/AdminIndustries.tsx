...
export const AdminIndustries: React.FC<AdminIndustriesProps> = ({
  ...
  const handleDelete = (id: string) => {
    if (typeof window !== 'undefined' && window.confirm(currentLang === 'ru' ? 'Удалить эту отрасль?' : 'Ushbu sohani o‘chirishni xohlaysizmi?')) {
  ...
  const handleSaveModal = () => {
    if (!editingIndustry) return;
    const nameObj = typeof editingIndustry.name === 'object' 
      ? editingIndustry.name 
      : { uz: editingIndustry.name, ru: editingIndustry.name };
  ...
  return (
    <div className="space-y-6">
      ...
      {isModalOpen && editingIndustry && (
        <div 
          className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-950/85 backdrop-blur-md overflow-y-auto animate-in fade-in duration-200"
          onClick={() => setIsModalOpen(false)}
        >
          <div 
            className="w-full max-w-2xl bg-gray-900 border border-gray-800 rounded-3xl p-6 space-y-5 shadow-2xl my-auto max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <div className="flex items-center justify-between pb-3 border-b border-gray-800">
              ...
              <button 
                type="button" 
                onClick={() => setIsModalOpen(false)}
                className="p-1.5 text-gray-400 hover:text-white rounded-xl hover:bg-gray-800 cursor-pointer transition"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-gray-800">
              <button
                type="button"
                onClick={() => setIsModalOpen(false)}
                className="px-4 py-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-semibold cursor-pointer transition"
              >
                Bekor qilish
              </button>
              <button
                ...
                className="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold shadow-lg shadow-blue-500/25 cursor-pointer transition"
              >
                Saqlash
              </button>
            </div>
