...
export const AdminCertificates: React.FC<AdminCertificatesProps> = ({
  ...
  const handleOgImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert('OG расмни юклашда хатолик: ' + (err?.message || 'Хатолик'));
    } finally {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gray-900/80 p-4 rounded-3xl border border-gray-800">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>Сертификатлар рўйхати ({certificates.length})</span>
          </button>
          ...
          >
            ...
            <span>SEO & OpenGraph созламалар</span>
          </button>
        </div>
        ...
        {activeTab === 'list' && (
          <button
            ...
            className="px-4 py-2 rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-lg shadow-blue-500/20"
          >
            <Plus className="w-4 h-4" />
            <span>Янги сертификат</span>
          </button>
        )}
      </div>
      ...
      {activeTab === 'list' && (
        <div className="space-y-4">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Қидириш (рақам, номи, ташкилот)..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-gray-900 border border-gray-800 text-xs text-white focus:outline-none focus:border-blue-500"
            />
          </div>
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div className="text-[11px] font-mono text-gray-500 pt-2 border-t border-gray-800 flex justify-between">
                      <span>Муддати: {cert.validUntil || '—'}</span>
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-gray-800/80">
                    <button
                      ...
                      className="flex-1 py-2 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-200 text-xs font-semibold transition flex items-center justify-center gap-1.5"
                    >
      ...
      {activeTab === 'seo' && (
        <form onSubmit={handleSaveSeo} className="p-6 rounded-3xl bg-gray-900 border border-gray-800 space-y-6 max-w-3xl">
          <div className="flex items-center justify-between border-b border-gray-800 pb-4">
            <div>
              <h3 className="text-base font-bold text-white">Сертификатлар саҳифаси SEO ва OpenGraph созламалари</h3>
              <p className="text-xs text-gray-400">Google, Yandex қидирув тизимлари ва Telegram / WhatsApp улашиш превьюси учун</p>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-300">
                Саҳифа Сарлавҳаси (Meta Title) [{seoLang.toUpperCase()}]
              </label>
              <input
                type="text"
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-300">
                Саҳифа Тавсифи (Meta Description) [{seoLang.toUpperCase()}]
              </label>
              <textarea
                rows={3}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-300">
                Калит сўзлар (Keywords) [{seoLang.toUpperCase()}]
              </label>
              <input
                type="text"
                value={(certSeo.keywords as any)?.[seoLang] || ''}
                onChange={(e) => handleSeoChange('keywords', e.target.value)}
                placeholder="сертификаты, метрология, госреестр узстандарт"
                className="w-full px-4 py-2.5 rounded-xl bg-gray-950 border border-gray-800 text-xs text-white focus:outline-none focus:border-blue-500"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="MAXTRON — Расмий сертификатлар"
                className="w-full px-4 py-2.5 rounded-xl bg-gray-950 border border-gray-800 text-xs text-white focus:outline-none focus:border-blue-500"
              />
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-300 flex items-center gap-1.5">
                <Share2 className="w-3.5 h-3.5 text-cyan-400" />
                <span>OpenGraph Description (Улашиш тавсифи) [{seoLang.toUpperCase()}]</span>
              </label>
              <textarea
                rows={2}
                value={(certSeo.ogDescription as any)?.[seoLang] || ''}
                onChange={(e) => handleSeoChange('ogDescription', e.target.value)}
                placeholder="MAXTRON расмий дистрибьюторлик ва Ўзстандарт сертификатлари..."
                className="w-full px-4 py-2.5 rounded-xl bg-gray-950 border border-gray-800 text-xs text-white focus:outline-none focus:border-blue-500"
              />
            </div>
            ...
            <div className="p-4 rounded-2xl bg-gray-950/80 border border-gray-800 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-gray-500 font-mono">1200x630 px тавсия этилади</span>
              </div>
                ...
                <div className="sm:col-span-8 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-semibold text-xs transition flex items-center gap-1.5 disabled:opacity-50"
                    >
                      {isUploadingOg ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Upload className="w-3.5 h-3.5" />}
                      <span>{isUploadingOg ? 'Юкланмоқда...' : 'Расм юклаш'}</span>
                    </button>
                    ...
                    {certSeo.ogImage && (
                      <button
                        ...
                        className="p-2 rounded-xl bg-gray-800 hover:bg-rose-500/20 text-gray-400 hover:text-rose-400 transition"
                        title="Расмни тозалаш"
                      >
                  ...
                  <input
                    type="text"
                    placeholder="Ёки тўғридан-тўғри расм ҳаволаси (URL)..."
          ...
          <div className="pt-3 border-t border-gray-800 flex justify-end">
            <button
              type="submit"
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-bold text-xs transition flex items-center gap-2 shadow-lg shadow-blue-500/25"
            >
