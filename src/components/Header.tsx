            ...
            <nav className="flex items-center gap-1 ml-2">
              {navLinks.map((link) => (
                <NavLink
                  key={link.to}
                  to={link.to}
                  end={link.to === '/' || link.to === `/${currentLang}`}
                  className={({ isActive }) =>
                    `px-3.5 py-1.5 text-xs font-semibold rounded-xl transition ${
              ...
              {headerCustomPages.map((page) => (
                <NavLink
                  key={page.id}
                  to={getLocalizedUrl(`/page/${page.slug}`)}
                  className={({ isActive }) =>
          ...
          <div className="grid grid-cols-1 gap-1">
            {navLinks.map((link) => (
              <NavLink
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={({ isActive }) =>
                  `px-4 py-2.5 rounded-xl text-xs font-semibold transition ${
